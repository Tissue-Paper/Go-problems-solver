...
int put_stone(int z, int color) {
  ...
  // コウの形の場合
  if (prisoner == 1 && possibility_of_ko && liberty == 1) {
    ko_z = provisional_ko_z;
  }
  else ko_z = 0;
...
// ダメの数とその座標を記録し、その個数を返す関数
int count_riberty(int z, int color, int first_time) {
  static int liberty;
  int new_z, i, j;
...
// 石を取り除き、その個数を返す関数
int take_stone(int z, int color, int first_time) {
  static int stone;
  int i;
...
// 死活判定の対象がすべて存在するなら1を、取られていたら0を返す関数
int whether_existence() {
  int i;
...
// 盤上にアタリになっている相手の石があれば、そのダメ座標を配列に保存し、その個数を返す関数
int atari() {
  int z, i;
...
// そこが眼であるかどうかを返す関数（簡易版）
int whether_its_eye(int z, int color) {
  int i, new_z;
...
int whether_its_eye_pro(int z, int color) {
  ...
  // その合計が４つなら、単眼の可能性あり
  if (each_number[color] + each_number[OUT] == 4) {
    each_number[color] = 0;
  ...
  // 合計が３つなら、複眼の可能性あり
  } else if (each_number[color] + each_number[OUT] == 3) {
    researched[next_z] = 1;
...
// 上下左右にある、target_stoneとOUTの合計数を返す
int kyusyo(int z) {
  int sum = 0, new_z, i;
...
int make_choices(int tekazu) {
  ...
  for (i = 0; i<atari_no_kazu; i++) {
    for (j = 0; j<KOUHO; j++) {
      if (place_atari[i] == coppy_canput[j]) {
        coppy_canput[j] = 0;  // Ａ配列と重複するＢ配列の要素を削除
        break;
      }
    }
  }
  ...
  c = 0;
  // もし対象石の手番なら、パスを選択肢に追加する
  if (turn == target_color) choices_data[tekazu][c++] = 1;

  for (i = 0; i<KOUHO; i++) {       // Ｂ配列の要素を１つずつ調べる
    ...
    if (board[z_index] == SPACE) {  // その地点が空点なら
      ...
      coppy_canput[i] = 0; // その地点に既に石があるなら、着手できないので削除
    }
  }
  ...
  for (i = 4; i>-1; i--)          // Ｂ配列の要素を、kyusyo関数の戻り値が大きい順に、Ｃ配列に移す
    for (j = 0; j<KOUHO; j++)
...
// 死活のプレイアウトを行う関数
int sikatsu_play_out(int tekazu_in_whole, int *tekazu_in_func) {
  int next_index, i;
...
// 木探索を行う関数
int search_result(int tekazu_in_whole, int count) {
...
// 同じ局面を繰り返していないか確認する関数
int check_repeat(int revel) {
  int i;
...
// 同一の局面で、同一の手を打とうとしていないか確認する関数
int check_repeat_2(int revel, int wish_z) {
  int i;
